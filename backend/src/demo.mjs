...
import { executeResult, nowIso, publicUser, queryOne } from "./database.mjs";
...
export async function ensureDemoData(db, config) {
  const now = nowIso();
  let user = await queryOne(db, "SELECT * FROM users WHERE id = ?", [DEMO_USER_ID]);
  if (!user) {
    await executeResult(db, `
      INSERT INTO users (id, email, password_hash, created_at)
      VALUES (?, ?, ?, ?)
    `, [DEMO_USER_ID, DEMO_EMAIL, bcrypt.hashSync("demo-password", 12), now]);
    user = await queryOne(db, "SELECT * FROM users WHERE id = ?", [DEMO_USER_ID]);
  }

  if (!(await queryOne(db, "SELECT id FROM sites WHERE id = ?", [DEMO_SITE_ID]))) {
    await executeResult(db, `
      INSERT INTO sites (id, user_id, name, slug, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `, [DEMO_SITE_ID, DEMO_USER_ID, "Demo 品牌展示站", "demo-brand-site", now, now]);
  }

  const rootPath = path.join(config.uploadRoot, DEMO_SITE_ID, DEMO_DEPLOYMENT_ID);
  await writeDemoFiles(rootPath);

  if (!(await queryOne(db, "SELECT id FROM deployments WHERE id = ?", [DEMO_DEPLOYMENT_ID]))) {
    await executeResult(db, `
      INSERT INTO deployments (id, site_id, version, original_name, root_path, file_count, total_bytes, created_at)
      VALUES (?, ?, 1, ?, ?, 2, 3200, ?)
    `, [DEMO_DEPLOYMENT_ID, DEMO_SITE_ID, "demo-site.zip", rootPath, now]);
  }

  await executeResult(
    db,
    "UPDATE sites SET active_deployment_id = ?, updated_at = ? WHERE id = ?",
    [DEMO_DEPLOYMENT_ID, now, DEMO_SITE_ID]
  );

  if (!(await queryOne(db, "SELECT id FROM domains WHERE id = ?", [DEMO_DOMAIN_ID]))) {
    await executeResult(db, `
      INSERT INTO domains (id, site_id, hostname, status, cname_target, last_checked_at, verified_at, created_at)
      VALUES (?, ?, ?, 'verified', ?, ?, ?, ?)
    `, [DEMO_DOMAIN_ID, DEMO_SITE_ID, "demo.example.com", config.cnameTarget, now, now, now]);
  }
